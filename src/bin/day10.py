...
print_justified_rectangle(dists)
...
while len(bfs) > 0:
    ...
    if curr_tile == "S":
        ...
        # Bottom
        if cur_row < (height - 1) and graph[cur_row + 1][cur_col] in ["|", "L", "J"]:
            bfs.append((cur_row + 1, cur_col, dist + 1))
        # Left
        if cur_col > 0 and graph[cur_row][cur_col - 1] in ["-", "L", "F"]:
            bfs.append((cur_row, cur_col - 1, dist + 1))
        # Right
        if cur_col < (width - 1) and graph[cur_row][cur_col + 1] in ["-", "J", "7"]:
            bfs.append((cur_row, cur_col + 1, dist + 1))
    if curr_tile == "|":
        # Top
