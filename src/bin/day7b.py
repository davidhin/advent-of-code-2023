...
def hand_type(hand):
    ...
    if sorted(hand_counts.values()) == [5]:
        return 7
    if sorted(hand_counts.values()) == [1, 4]:
        if "J" in hand_counts:
            return 7
        return 6
    if sorted(hand_counts.values()) == [2, 3]:
        if "J" in hand_counts:
            return 7
        return 5
    if sorted(hand_counts.values()) == [1, 1, 3]:
        if "J" in hand_counts:
            return 6
        return 4
    if sorted(hand_counts.values()) == [1, 2, 2]:
        if hand_counts["J"] == 1:
            return 5
        if hand_counts["J"] == 2:
            return 6
        return 3
    if sorted(hand_counts.values()) == [1, 1, 1, 2]:
        if "J" in hand_counts:
            return 4
        return 2
    if "J" in hand_counts:
